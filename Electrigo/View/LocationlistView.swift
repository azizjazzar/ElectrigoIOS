...
struct LocationDetailView: View {
    ...
    private func starType(index: Int) -> String {
        let totalRating = self.location.reviews.reduce(0) { $0 + $1.rating }
        let averageRating = totalRating / self.location.reviews.count

        return index < averageRating ? "star.fill" : "star"
    }
//
    var body: some View {
        ScrollView {
